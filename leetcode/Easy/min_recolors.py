...
# is either 'W' or 'B', representing the color of the ith block. 
# The characters 'W' and 'B' denote the colors white and black, respectively.

# You are also given an integer k, which is the desired number 
# of consecutive black blocks. In one operation, you can recolor a 
# white block such that it becomes a black block.
...
# 0th, 3rd, and 4th blocks so that blocks = "BBBBBBBWBW". 
...
def minimumRecolors(blocks, k):
    ...
    for i in range(len(blocks)-k+1):
        num = min(num, blocks[i:i+k].count('W'))
    return num
...
if __name__ == '__main__':
    ...
    print(minimumRecolors("BWWWBB", 6))  # 3
