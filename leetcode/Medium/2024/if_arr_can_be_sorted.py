...
def canSortArray(nums=[]):
    bitnums = []
    bits = [nums[0]]
    # count_bits = sum([int(x) for x in bin(nums[0]) if x.isdigit()])
    count_bits = bin(nums[0]).count('1')
    for n in nums[1:]:
        count = bin(n).count('1')
        if count == count_bits:
            bits.append(n)
            ...
            count_bits = count
    bitnums.append(bits)
    for i in range(len(bitnums) - 1):
        if max(bitnums[i]) > min(bitnums[i+1]):
            return False
    return True
...
if __name__ == '__main__':
    ...
    # print(sum([int(x) for x in bin(10) if x.isdigit()]))
